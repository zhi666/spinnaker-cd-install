...
NODES="k8s-node02 k8s-master02 k8s-master01"
...
function GetImages(){
    ...
    do
        ...
        do 
           ...
           ssh ${node} "ctr -n k8s.io i pull ${T_REGISTRY}/${image}"
           echo  -e "\033[32m ${node} ---> tag ---> ${image} \033[0m"
           ssh ${node} "ctr  -n k8s.io image tag ${T_REGISTRY}/${image} ${S_REGISTRY}/${image}"
        done
    done
    for node in ${NODES}
    do
       echo -e "\033[43;34m =====${node}===镜像信息===== \033[0m"
       ssh ${node} "ctr -n k8s.io i ls | grep 'spinnaker-marketplace' "
    done
